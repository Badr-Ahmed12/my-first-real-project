...
export default function ProjectPage() {
  const projectInfo: ProjectInfoItem[] = [
    ...
    {
      title: "Project link:",
      content: <Link to="https://www.flatheme.net">www.flatheme.net</Link>,
    },
    {
      title: "Duration:",
      content: "124 Hours",
  ...
  return (
    <div className="z-10 relative">
      ...
      {/* Main Content */}
      <main className="container mx-auto max-w-[1320px] px-5 md:px-10 xl:px-5">
        <ProjectHero />
...
// Sub-components
const Header = () => (
  <header className="fixed top-0 left-0 w-full py-5 bg-darkBg/90 backdrop-blur-md">
...
const NavButtons = () => (
  <ul className="flex space-x-2">
    <NavButton text="Let's Talk" color="white" hasIcon />
    <NavButton text="Menu" color="black" hasIcon />
  </ul>
);

const NavButton = ({ text, color, hasIcon }: { text: string; color: string; hasIcon: boolean }) => (
  <li className="list-none inline-block">
    <button className={`nav-button ${color === 'white' ? 'bg-white/15 text-white' : 'bg-white text-black'}`}>
      <HoverText text={text} />
      {hasIcon && <span className="nav-button-icon" />}
    </button>
  </li>
);
...
const ProjectNavigation = () => (
  <div className="flex justify-between mt-12">
    <ProjectNavButton direction="prev" />
    <ProjectNavButton direction="next" />
  </div>
);

const ProjectNavButton = ({ direction }: { direction: string }) => (
  <a className="project-nav-button">
